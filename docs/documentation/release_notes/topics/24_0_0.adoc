...
From this release onward it is required that you access Keycloak from a https://developer.mozilla.org/en-US/docs/Web/Security/Secure_Contexts[secure context]. This change is necessary as many Web APIs and features are accessible only in a secure context, including those required for upcoming changes related to the https://privacysandbox.com/[Privacy Sandbox initiative].

TODO: Add screenshot of the error page

For more details consult the link:{upgradingguide_link}[{upgradingguide_name}].
