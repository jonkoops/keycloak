...
As part of the https://privacysandbox.com/[Privacy Sandbox] effort, Keycloak is making changes to the way cookies are handled. specifically we are preparing for the https://developers.google.com/privacy-sandbox/3pcd/prepare/prepare-for-phaseout[third-party cookie phaseout], and are looking to improve the consistency, privacy and security of how cookies are handled by default within Keycloak.
...
You can restore this behavior by configuring the https://www.keycloak.org/server/all-provider-config#_cookie[cookie provider]. For example:
...
This is however considered a temporary workaround, and will be removed in a future release.
