...
= Secure Context now required

It is now required that you access Keycloak from a https://developer.mozilla.org/en-US/docs/Web/Security/Secure_Contexts[secure context], this is required both for local development, as well as running Keycloak in production.

For local development ensure you are accessing Keycloak from `http://127.0.0.1`, `http://localhost` or a `http://*.localhost` URL (e.g. `http://my-keycloak-server.localhost/`). For production environments make sure that you are serving Keycloak from a secure `https://` connection.

If you are unsure if your Keycloak instance is running in a secure context consult https://developer.mozilla.org/en-US/docs/Web/Security/Secure_Contexts#when_is_a_context_considered_secure["When is a context considered secure?"].
