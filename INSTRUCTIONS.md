...
**Known limitation**: Clients with `authorizationServicesEnabled: true` should be created using `adminClient.createClient()` rather than passing them to `createTestBed()`. When authorization services are enabled via realm creation, the default resources and policies that Keycloak normally creates are not properly initialized. This appears to be a server-side limitation with how clients are processed during realm import vs. individual client creation.
